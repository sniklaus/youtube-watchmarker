...
let refresh = function () {
  ...
  for (let objVideo of objVideos) {
    ...
    if (Object.hasOwn(intWatchdate, strIdent) === true) {
      continue;
    }

    for (
      let intTitle = 0, objTitle = objVideo.parentNode;
      intTitle < 5;
      intTitle += 1, objTitle = objTitle.parentNode
    ) {
  ...
  strLastchange =
    window.location.href + ":" + window.document.title + ":" + objVideos.length;
};

let mark = function (objVideo, strIdent) {
  if (
    Object.hasOwn(intWatchdate, strIdent) === true &&
...
chrome.runtime.onMessage.addListener(
  function (objData, objSender, funcResponse) {
...
window.setInterval(function () {
  if (document.hidden === true) {
    ...
    strLastchange ===
    window.location.href + ":" + window.document.title + ":" + videos("").length
  ) {
    return;
  }
